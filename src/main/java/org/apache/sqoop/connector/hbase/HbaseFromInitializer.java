...
import org.apache.sqoop.connector.hbase.util.SqlTypesUtils;
...
public class HbaseFromInitializer extends Initializer<LinkConfiguration, FromJobConfiguration> {

  private static final Logger LOG =
    Logger.getLogger(HbaseFromInitializer.class);
  ...
  public Schema getSchema(InitializerContext context, LinkConfiguration linkConfig, FromJobConfiguration fromJobConfig) {
    ...
    try {
      rs = executor.executeQuery(
        context.getString(HbaseConnectorConstants.CONNECTOR_JDBC_FROM_DATA_SQL)
  ...
  private void configurePartitionProperties(MutableContext context, LinkConfiguration linkConfig, FromJobConfiguration jobConf) throws SQLException {
    ...
    if(incrementalImport) {
      ...
      try {
        ...
        context.setString(HbaseConnectorConstants.CONNECTOR_JDBC_LAST_INCREMENTAL_VALUE, incrementalMaxValue);
  ...
  private void configureTableProperties(MutableContext context, LinkConfiguration linkConfig, FromJobConfiguration fromJobConfig) {
    ...
    if (tableName != null && tableSql != null) {
      ...
      if (tableColumns == null) {
        ...
        builder.append(HbaseConnectorConstants.SQL_CONDITIONS_TOKEN);
        dataSql = builder.toString();

        String[] queryColumns = executor.getQueryColumns(dataSql.replace(
            HbaseConnectorConstants.SQL_CONDITIONS_TOKEN, "1 = 0"));
      ...
      assert tableSql.contains(HbaseConnectorConstants.SQL_CONDITIONS_TOKEN);
      ...
      if (tableColumns == null) {
        ...
        builder.append(executor.qualify(
            columns[0], HbaseConnectorConstants.SUBQUERY_ALIAS));
        for (int i = 1; i < columns.length; i++) {
          builder.append(",");
          builder.append(executor.qualify(
              columns[i], HbaseConnectorConstants.SUBQUERY_ALIAS));
        }
        builder.append(" FROM ");
        builder.append("(");
        builder.append(tableSql);
        builder.append(") ");
        builder.append(HbaseConnectorConstants.SUBQUERY_ALIAS);
        dataSql = builder.toString();
    ...
    context.setString(HbaseConnectorConstants.CONNECTOR_JDBC_FROM_DATA_SQL, dataSql);
